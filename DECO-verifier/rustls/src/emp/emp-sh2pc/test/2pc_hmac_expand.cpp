...
void test(char** argv) {
	...
	stringstream ss;
	// Input share
	string share_state_ipad_bin = argv[3];
	string share_state_opad_bin = argv[4];
	...
	for(int i = 0; i < 256; ++i) {
		in[i] = (share_state_ipad_bin[i] == '1');
		cout << in[i];
	}

 	for(int i = 256; i < 512; ++i) {
		in[i] = (share_state_opad_bin[i-256] == '1');
		cout << in[i];
	}
	cout << endl;
	...
	Integer a(768, in, ALICE);
	Integer b(1024, in_b, BOB);
