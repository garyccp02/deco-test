...
pub struct ClientConfig {
    ...
    /// 
    pub kx_groups: Vec<&'static SupportedKxGroup>, // delete pub(super)
...
impl ClientConnection {
    ...
    fn keygen_by_new_inner(
        config: Arc<ClientConfig>,
        name: ServerName,
        extra_exts: Vec<ClientExtension>,
        proto: Protocol,
    ) -> kx::KeyExchange {
        let mut common_state = CommonState::new(config.max_fragment_size, true).unwrap();
        common_state.protocol = proto;
        let mut data = ClientConnectionData::new();
        ...
        hs::keygen_by_start_handshake(name, extra_exts, config, &mut cx)
    }
...
pub trait ClientQuicExt {
    ...
    // ) -> kx::KeyExchange {
    //     if !config.supports_version(ProtocolVersion::TLSv1_3) {
    //         println!("[ERROR] TLS 1.3 support is required for QUIC");
    //         // return Err(Error::General(
    //         //     "TLS 1.3 support is required for QUIC".into(),
    //         // ));
    //     }
    ...
    //     ClientConnection::keygen_by_new_inner(config, name, vec![ext], Protocol::Quic)
    // }
}
