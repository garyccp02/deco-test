...
pub use crate::msgs::persist; // add a pub
use crate::rand;
...
mod client_hello {
    ...
    fn emit_server_hello(
        ...
        // this share is from client: g^(x+v)
        println!("\n[server side: client key share] {:?}", &share.payload.0);

//------------------------------------------------------------------------
        let server_keyexchange = kx::KeyExchange::choose(share.group, &config.kx_groups)
            .and_then(kx::KeyExchange::start)
            .ok_or(Error::FailedToGetRandomBytes)?;
        let bytes_print = &server_keyexchange.pubkey.bytes.bytes;

        // println!("\n[server tls.rs] server key_share (pubkey.bytes.bytes): {:?}", bytes_print);
//------------------------------------------------------------------------
        ...
        // Do key exchange
        let kxr = server_keyexchange.complete(&share.payload.0)
            .ok_or_else(|| Error::PeerMisbehavedError("key exchange failed".to_string()))?;
        
        println!("\n[server: DH keyshare] {:?}",&kxr.shared_secret);
