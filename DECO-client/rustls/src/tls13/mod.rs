...
impl Tls13CipherSuite {
    pub(crate) fn derive_encrypter(&self, secret: &hkdf::Prk) -> Box<dyn MessageEncrypter> {
        ...
        println!("encrypter key: {:?}", &key);
        // println!("encrypter iv: {:?}", &iv);
    ...
    pub fn derive_decrypter(&self, secret: &hkdf::Prk) -> Box<dyn MessageDecrypter> {
        ...
        // println!("decrypter key: {:?}", &key.inner);
        println!("decrypter iv: {:?}", &iv.0);
    ...
    // [DECO]
    // Handle tk_sapp with 2PC-HMAC
    pub fn derive_decrypter_deco_tksapp(&self, secret: &hkdf::Prk) -> Box<dyn MessageDecrypter> 
    {

        let key = crate::tls13::key_schedule::derive_traffic_key_deco_tksapp(secret, self.common.aead_algorithm);
        let iv = crate::tls13::key_schedule::derive_traffic_iv_deco_tksapp(secret);

        // println!("decrypter key: {:?}", key.0);
        println!("decrypter iv: {:?}", iv.0);
...
// // [DECO]
// pub fn derive_decrypter_deco_dumpy(secret: &hkdf::Prk) -> Tls13MessageDecrypter {
//     let key = derive_traffic_key(secret, &aead::AES_128_GCM);
//     let iv = derive_traffic_iv(secret);
...
impl MessageDecrypter for Tls13MessageDecrypter {
    fn decrypt(&self, mut msg: OpaqueMessage, seq: u64) -> Result<PlainMessage, Error> {
        ...
        // // [DECO] begin
        // println!("Hi decrypt!");
        // let dumpy_key = ring::hmac::get_dumpy_key(ring::hmac::HMAC_SHA256);
        // let dumpy_key_Prk = ring::hkdf::Prk::hmac_key_to_prk(dumpy_key.clone());
        // let decrypter: Tls13MessageDecrypter = derive_decrypter_deco_dumpy(&dumpy_key_Prk);
        // assert!(decrypter.dec_key == self.dec_key);
        // // [DECO] end
