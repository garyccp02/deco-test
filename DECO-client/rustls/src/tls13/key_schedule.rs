...
impl KeyScheduleEarly {
    ...
    // [DECO]
    pub(crate) fn into_handshake_2pc_hmac_hs_msg(
        mut self, 
        secret: &[u8],
        input: String
    ) -> KeyScheduleHandshakeStart {
        println!("BBBBBBBBBBBBBBBBBBBBBBBBBBc");
        self.ks.input_secret_2pc_hmac_hs_msg(input.clone());
        KeyScheduleHandshakeStart { ks: self.ks } 
    }
}
...
impl KeyScheduleHandshakeStart {
    // [DECO] for CHTS and SHTS
    pub(crate) fn derive_handshake_secrets_deco(
        self,
        hs_hash: Digest,
        key_log: &dyn KeyLog,
        client_random: &[u8; 32],
        ...
        println!("=====================================");
        println!("derive_handshake_secrets_deco");
        println!("hs_hash: {:?}", hs_hash);

        println!("below is client secret");
        println!("~~~~ CHTS ~~~~");
        let client_secret = self.ks.derive_logged_secret_deco(
            SecretKind::ClientHandshakeTrafficSecret,
            hs_hash.as_ref(),
            key_log,
            client_random,
        );
        hkdf::Prk::read(client_secret.clone());
        println!("-------------------");

        println!("below is server secret");
        println!("~~~~ SHTS ~~~~");
        let server_secret = self.ks.derive_logged_secret_deco(
            SecretKind::ServerHandshakeTrafficSecret,
            hs_hash.as_ref(),
            key_log,
            client_random,
        );
        hkdf::Prk::read(server_secret.clone());
        println!("-------------------");
    ...
    pub(crate) fn derive_handshake_secrets(
        ...
        println!("=====================================");
        println!("derive_handshake_secrets");
        println!("hs_hash: {:?}", hs_hash);
...
impl KeyScheduleHandshake {
    ...
    pub(crate) fn into_traffic_with_client_finished_pending(
        ...
        println!("into_traffic_with_client_finished_pending");
    ...
    pub(crate) fn into_traffic_with_client_finished_pending_deco(
        self,
        hs_hash: Digest,
        key_log: &dyn KeyLog,
        client_random: &[u8; 32],
        ...
        println!("into_traffic_with_client_finished_pending_deco");
        let traffic = KeyScheduleTraffic::new_deco(self.ks, hs_hash, key_log, client_random);
...
impl KeyScheduleTraffic {
    fn new(
        ...
        println!("{:?}", current_client_traffic_secret);
    ...
    fn new_deco(
        ...
        println!("~~~~ CATS ~~~~");
        let current_client_traffic_secret = ks.derive_logged_secret_deco(
            SecretKind::ClientApplicationTrafficSecret,
            hs_hash.as_ref(),
            key_log,
            client_random,
        ...
        println!("~~~~ SATS ~~~~");
        let current_server_traffic_secret = ks.derive_logged_secret_deco(
            SecretKind::ServerApplicationTrafficSecret,
            hs_hash.as_ref(),
            key_log,
            client_random,
        );
        hkdf::Prk::read(current_server_traffic_secret.clone());
        // let current_server_traffic_secret = ks.derive_logged_secret(
        //     SecretKind::ServerApplicationTrafficSecret,
        //     hs_hash.as_ref(),
        //     key_log,
        ...
        println!("~~~~ EMS ~~~~");
        let current_exporter_secret = ks.derive_logged_secret_deco(
            SecretKind::ExporterMasterSecret,
            hs_hash.as_ref(),
            key_log,
            client_random,
...
impl KeySchedule {
    ...
    // [DECO] for dHS and MS
    fn input_empty_deco(&mut self) {
        let zeroes = [0u8; digest::MAX_OUTPUT_LEN];
        println!("input_empty_deco");
        self.input_secret_deco(&zeroes[..self.algorithm.len()]);
    }
    ...
    fn input_secret(&mut self, secret: &[u8]) {
        ...
        println!("after salt generation");
        self.current = salt.extract(secret); // HS or MS
        println!("HS: {:?}", &self.current);
    }

    /// [DECO] Input the given secret with 2PC-HMAC
    fn input_secret_2pc_hmac_hs_msg(&mut self, input: String) {
        let salt: hkdf::Salt = self.derive_for_empty_hash(SecretKind::DerivedSecret);
        ...
        println!("after salt generation");
        // self.current = 
        // salt.extract_2pc_hmac_hs_msg(input.clone(), crate::TARGET_IP); // HS
        salt.extract_2pc_hmac_hs_msg(input.clone(), &target_ip.as_str()); // HS
        // println!("HS: {:?}", &self.current);
    }

    // [DECO] for dHS and MS
    fn input_secret_deco(&mut self, secret: &[u8]) {

        let (my_ip_port, target_ip, target_ip_port) = crate::get_ip();

        // Send the proceed messgae to verifier
        use std::net::TcpStream;
        use std::io::Write;
        let signal_to_verifier: Vec<u8> = String::from("continue dHS").as_bytes().to_vec();
        ...
        let prk: hkdf::Prk = self.derive_for_empty_hash_deco(SecretKind::DerivedSecret); // dHS
        let salt: hkdf::Salt = hkdf::Salt::prk_to_salt(prk);

        println!("secret before MS: length: {:?}; content: {:?}", secret.len(), secret);

        // The message input with padding.
        let mut info_test: Vec<u8> = Vec::new();
        for i in 0..secret.len() {
            info_test.push(secret[i]);
        }
        let length = (info_test.len()*8 + 512);
        let length_be_byte = u16::to_be_bytes(length as u16);
        println!("length: {:?} {:?}", length, length_be_byte);

        info_test.push(128u8);

        let num_of_zero_bytes = 64 - info_test.len() - 2;
        ...
        info_test.push(length_be_byte[0]);
        info_test.push(length_be_byte[1]);
        assert!(info_test.len() == 64);

        println!("info_test: {:?}", info_test);

        println!("after salt generation");

        let (my_ip_port, target_ip, target_ip_port) = crate::get_ip();
        self.current = hkdf::Prk::expand_deco(
            info_test,
            &my_ip_port.as_str(),
        ...
        // self.current = hkdf::Prk::expand_deco(
        //     info_test,
        //     crate::MY_IP_WITH_PORT,
        //     crate::TARGET_IP_WITH_PORT,
        //     crate::TARGET_IP
        // );

        // self.current = salt.extract(secret); // MS
        println!("MS: {:?}", &self.current);
    }
    ...
    fn derive_logged_secret(
        ...
        println!("before derive");
        self.derive(self.algorithm, kind, hs_hash)
    }

    // [DECO]
    /// Derive a secret of given `kind`, using current handshake hash `hs_hash`.
    fn derive_deco<L>(&self, key_type: L, kind: SecretKind, hs_hash: &[u8]) -> hkdf::Prk 
    where
        L: hkdf::KeyType,
    {
        hkdf_expand_deco(&self.current, key_type, kind.to_bytes(), hs_hash)
    }

    // [DECO]
    fn derive_logged_secret_deco(
        &self,
        kind: SecretKind,
        hs_hash: &[u8],
        key_log: &dyn KeyLog,
        ...
        println!("derive_logged_secret_deco");
        // It will do duplicated extract??
        let log_label = kind
            .log_label()
            .expect("not a loggable secret");
        if key_log.will_log(log_label) {
        ...
        println!("before derive_deco");
        self.derive_deco(self.algorithm, kind, hs_hash)
    }
    ...
    fn derive_for_empty_hash_deco(&self, kind: SecretKind) -> hkdf::Prk {
        println!("derive_for_empty_hash_deco");
        let digest_alg = self
            .algorithm
            .hmac_algorithm()
            .digest_algorithm();
        ...
        let empty_hash = digest::digest(digest_alg, &[]);
        println!("~~~~ dHS ~~~~");
        self.derive_deco(self.algorithm, kind, empty_hash.as_ref())
    }
...
// [DECO]
pub(crate) fn hkdf_expand_deco<L>(secret: &hkdf::Prk, key_type: L, label: &[u8], context: &[u8]) -> hkdf::Prk 
where
    L: hkdf::KeyType,
{
    hkdf_expand_info_deco(secret, key_type, label, context)
}

// [DECO]
fn hkdf_expand_info_deco<L>(
    secret: &hkdf::Prk,
    key_type: L,
    label: &[u8],
    context: &[u8],
...
// [DECO]
pub(crate) fn hkdf_expand_deco_tksapp<L>(secret: &hkdf::Prk, key_type: L, label: &[u8], context: &[u8])
where
    L: hkdf::KeyType,
{
    hkdf_expand_info_deco_tksapp(secret, key_type, label, context);
}

// [DECO]
fn hkdf_expand_info_deco_tksapp<L>(
    secret: &hkdf::Prk,
    key_type: L,
    label: &[u8],
    context: &[u8],
    ...
    L: hkdf::KeyType,
{
    ...
    hkdf::expand_deco_tksapp(info_test);
}
...
pub(crate) fn derive_traffic_key_deco_tksapp(
    secret: &hkdf::Prk,
    aead_algorithm: &'static aead::Algorithm,
) -> aead::UnboundKey {
    println!("secret: {:?}", secret);
    println!("aead_algorithm: {:?}", aead_algorithm);
    hkdf_expand_deco_tksapp(secret, aead_algorithm, b"key", &[]);
    aead::UnboundKey::new(aead_algorithm, &[0u8; 16]).unwrap()
}

pub(crate) fn derive_traffic_iv_deco_tksapp(secret: &hkdf::Prk) -> Iv {
    println!("secret: {:?}", secret);
    hkdf_expand_deco_tksapp(secret, IvLen, b"iv", &[]);
    Iv([0; 12])
}
