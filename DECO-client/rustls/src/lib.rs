...
#![forbid( unused_must_use, unstable_features)]
#![deny(
...
pub fn uint_to_yint(u_int: &BigInt) -> BigInt {
    ...
    let u_plus_1 = u_int + &BigInt::one();
    let u_minus_1 = u_int - &BigInt::one();
    let u_plus_1_inv = BigInt::mod_inv(&u_plus_1, &p).unwrap();
    BigInt::mod_mul(&u_minus_1, &u_plus_1_inv, &p)
    }

pub fn yint_to_uint(y_int: &BigInt) -> BigInt {
    // prime order of 25519 = 2^255 - 19
    let p: BigInt = BigInt::from_str_radix(
        // "115792089237316195423570985008687907853269984665640564039457584007908834671663",
        "57896044618658097711785492504343953926634992332820282019728792003956564819949", 
    ...
    let one_plus_y = &BigInt::one() + y_int;
    let one_minus_y = &BigInt::one() - y_int;
    let one_minus_y_inv = BigInt::mod_inv(&one_minus_y, &p).unwrap();
    BigInt::mod_mul(&one_plus_y, &one_minus_y_inv, &p)
}

// pub const VERIFIER_IP: &str = "127.0.0.1";
// pub const VERIFIER_IP_WITH_PORT: &str = "127.0.0.1:8080";

// pub const MY_IP_WITH_PORT: &str = "192.168.0.113:8081";
// pub const TARGET_IP: &str = "192.168.0.103";
// pub const TARGET_IP_WITH_PORT: &str = "192.168.0.103:8080";

// pub const MY_IP_WITH_PORT: &str = "127.0.0.1:8081";
// pub const TARGET_IP: &str = "127.0.0.1";
// pub const TARGET_IP_WITH_PORT: &str = "127.0.0.1:8080";
...
pub fn get_ip() -> (String, String, String) {
    ...
    let mut my_ip_port_fs = "./rustls/src/ip/my_ip_with_port.txt";
    let mut target_ip_fs = "./rustls/src/ip/target_ip.txt";
    let mut target_ip_port_fs = "./rustls/src/ip/target_ip_with_port.txt";

    let mut my_ip_port = fs::read_to_string(my_ip_port_fs).expect("failed reading");
    let mut target_ip = fs::read_to_string(target_ip_fs).expect("failed reading");
    let mut target_ip_port = fs::read_to_string(target_ip_port_fs).expect("failed reading");

    (
        my_ip_port.trim().to_string(), 
        target_ip.trim().to_string(), 
        target_ip_port.trim().to_string()
    )
}