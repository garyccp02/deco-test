...
pub fn ECtF_Paillier(
    x1: BigInt, //client u
    y1: BigInt, // client v
    x2: BigInt, // verifier u
    y2: BigInt, // verifier v
) -> (BigInt, BigInt) {
...
pub fn ectf(
    ...
    let U_xy_hex = x1.clone().to_hex();
    println!("\nU_xy hex: {}\n", U_xy_hex);

    // ==================== Step 1 ==================== 
    // Prover
    let Cx = Paillier::encrypt(&ek,RawPlaintext::from(&x1)).0.to_bytes();
    let Cy = Paillier::encrypt(&ek,RawPlaintext::from(&y1)).0.to_bytes();
    let r1 = BigInt::sample_below(&p); 
    let Cr = Paillier::encrypt(&ek,RawPlaintext::from(&r1)).0.to_bytes();
    let y1r1 = BigInt::mod_mul(&y1, &r1, &p);
    let Cry = Paillier::encrypt(&ek, RawPlaintext::from(&y1r1)).0.to_bytes();
    ...
    let ek_n = ek.n.clone().to_bytes();
    stream.write(&ek_n).unwrap();

    // let mut stream = TcpStream::connect(crate::TARGET_IP_WITH_PORT).unwrap();
    let mut stream = TcpStream::connect(&target_ip_port.as_str()).unwrap();
    let ek_nn = ek.nn.clone().to_bytes();
    stream.write(&ek_nn).unwrap();

    // send Cx Cy Cr Cry to verifier
    // let mut stream = TcpStream::connect(crate::TARGET_IP_WITH_PORT).unwrap();
    let mut stream = TcpStream::connect(&target_ip_port.as_str()).unwrap();
    stream.write(&Cx).unwrap();

    // let mut stream = TcpStream::connect(crate::TARGET_IP_WITH_PORT).unwrap();
    let mut stream = TcpStream::connect(&target_ip_port.as_str()).unwrap();
    stream.write(&Cy).unwrap();
    ...
    // let mut stream = TcpStream::connect(crate::TARGET_IP_WITH_PORT).unwrap();
    let mut stream = TcpStream::connect(&target_ip_port.as_str()).unwrap();
    stream.write(&Cry).unwrap();

    let Cx = RawCiphertext::from(BigInt::from_bytes(&Cx));
    let Cy = RawCiphertext::from(BigInt::from_bytes(&Cy));
    let Cr = RawCiphertext::from(BigInt::from_bytes(&Cr));
    let Cry = RawCiphertext::from(BigInt::from_bytes(&Cry));
    ...
    // recieve C_beta, C_gamma, delta2, C_alpha from client
    fn get_ciphertext_i(mut stream: &TcpStream) 
    -> RawCiphertext {
    ...
    fn get_delta2(mut stream: &TcpStream) 
    -> BigInt {
        let mut buf = [0; 256]; 
        let read_bytes = stream.read(&mut buf).unwrap(); // read g^v
        let cipher = &buf[..read_bytes].to_vec();
    ...
    // let mut stream = TcpStream::connect(crate::TARGET_IP_WITH_PORT).unwrap();
    let mut stream = TcpStream::connect(&target_ip_port.as_str()).unwrap();
    let C_beta = get_ciphertext_i(&stream);

    // let mut stream = TcpStream::connect(crate::TARGET_IP_WITH_PORT).unwrap();
    let mut stream = TcpStream::connect(&target_ip_port.as_str()).unwrap();
    let C_gamma = get_ciphertext_i(&stream);    

    // let mut stream = TcpStream::connect(crate::TARGET_IP_WITH_PORT).unwrap();
    let mut stream = TcpStream::connect(&target_ip_port.as_str()).unwrap();
    let C_alpha = get_ciphertext_i(&stream);

    // let mut stream = TcpStream::connect(crate::TARGET_IP_WITH_PORT).unwrap();
    let mut stream = TcpStream::connect(&target_ip_port.as_str()).unwrap();
    let delta2 = get_delta2(&stream);

    println!("\nreceive C_beta: {:?}", &C_beta);
    println!("\nreceive C_gamma: {:?}", &C_gamma);
    println!("\nreceive C_alpha: {:?}", &C_alpha);
    println!("\nreceive delta2: {:?}", &delta2);
...
// ==================== Step 3 ==================== 
    ...
    // send delta1 to verifier
    // let mut stream = TcpStream::connect(crate::TARGET_IP_WITH_PORT).unwrap();
    let mut stream = TcpStream::connect(&target_ip_port.as_str()).unwrap();
    println!("generate delta1: {}", &delta1);
    stream.write(&delta1.to_bytes()).unwrap();

    // fn get_s2(mut stream: &TcpStream) 
    // -> BigInt {
    //     let mut buf = [0; 256]; 
    //     let read_bytes = stream.read(&mut buf).unwrap(); // read g^v
    //     let cipher = &buf[..read_bytes].to_vec();
    ...
    // let s2 = get_s2(&stream);

    // // this line is for test, we need a 2PC here in real case
    // let u_integer = BigInt::mod_add(&s1, &s2, &p); // x_k = x1 + x2 mod p

    // println!("u = s1 + s2 mod p = {}", &u_integer);
