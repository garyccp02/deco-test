...
impl ServerCertVerifier for WebPkiVerifier {
    ...
    fn verify_server_cert(
        ...
        println!("Progress check 3.1");
        let (cert, chain, trustroots) = prepare(end_entity, intermediates, &self.roots)?;
        println!("Progress check 3.2");
        let webpki_now = webpki::Time::try_from(now).map_err(|_| Error::FailedToGetCurrentTime)?;

        println!("Progress check 3.3");
        let ServerName::DnsName(dns_name) = server_name;

        println!("Progress check 3.4");
        let cert = cert
        ...
        println!("Progress check 3.5");
        if let Some(policy) = &self.ct_policy {
            println!("Progress check 3.5 inside if");
            policy.verify(end_entity, now, scts)?;
        }

        println!("Progress check 3.6");
        if !ocsp_response.is_empty() {
            println!("Progress check 3.6 inside if");
            trace!("Unvalidated OCSP response: {:?}", ocsp_response.to_vec());
        }

        println!("Progress check 3.7");
