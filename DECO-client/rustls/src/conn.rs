...
impl<Data> ConnectionCommon<Data> {
    ...
    fn process_msg(
        ...
        let encrypted_msg: OpaqueMessage = msg.clone(); // added

        println!("msg: {:?}", msg);
        println!("encrypted_msg: {:?}", encrypted_msg);
        println!("encrypted_msg.version: {:?}", encrypted_msg.version);

        // For TLS 1.2 Abort
        // Note: Even under TLS 1.3, it is stated as a TLS 1.2 message.
        if encrypted_msg.version == ProtocolVersion::TLSv1_2 
        && ((args[10] == "TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256" || args[4] == "TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256" )
        || (args[10] == "TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256" || args[4] == "TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256" ))
        && msg.typ == ContentType::Alert 
        {

            println!("********** Abort (From TLS1.2 check) ***********");

            // Notify verifier
            let signal_to_verifier: Vec<u8> = String::from("abort").as_bytes().to_vec();
            let mut stream = TcpStream::connect(target_ip_port).unwrap();
            stream.write(&signal_to_verifier).unwrap();

            // Abort
            use crate::Error::CorruptMessage;
            return Err(CorruptMessage);
        }

        let mut is_decrypt_error = false;
        // Decrypt if demanded by current state.
        println!("Decrypt if demanded by current state.");
            ...
            .is_decrypting()
        {
            true => {
                ...
                fn get_payload(mut stream: &TcpStream) 
                ->  Vec<u8> {
                    use std::io::Read;
                    let mut buf = [0; 16]; 
                ...
                println!("match true!!!!!"); // still may handshake_joiner == True
                let seq = self.common_state.record_layer.read_seq;
                println!("seq: {}", seq);
                let ret = match self.common_state.decrypt_incoming(msg.clone()) {
                    Ok(plain_tex) => plain_tex,
                    Err(_) => {
                        println!("{:?}", args[10]);
                        if encrypted_msg.version == ProtocolVersion::TLSv1_2 
                        && ((args[10] == "TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256" || args[4] == "TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256" )
                        ...
                        {
                            println!("TLS 1.2 is_decrypt_error handling...");

                            // Client ask verifier what is he doing.
                            let signal_to_verifier: Vec<u8> = String::from("Hey").as_bytes().to_vec();
                            ...
                            if client_message == String::from("After cf").as_bytes().to_vec() {
                                println!("After client finished!");
                            }

                            // send msg to verifier for decryption
                            let mut stream = TcpStream::connect(&target_ip_port.as_str()).unwrap();
                            stream.write(&msg.payload.0).unwrap();

                            // send msg seq to verifier for decryption
                            let mut stream = TcpStream::connect(&target_ip_port.as_str()).unwrap();
                            stream.write(&seq.to_be_bytes()).unwrap();
                        
                            // Verifier reply
                            let mut stream = TcpStream::connect(&target_ip_port.as_str()).unwrap();
                            let verifier_decrypt_payload = get_payload(&stream);
                            println!("verifier_decrypt_payload: {:?}", verifier_decrypt_payload);

                            // Verifier decrypt
                            let plainmsg: PlainMessage = PlainMessage {
                                typ: ContentType::Handshake,
                                version: ProtocolVersion::TLSv1_2,
                                payload: Payload {0: verifier_decrypt_payload}
                            };
                ...
                println!("ret: {:?}", ret);
                // shall be dealt in verifier
                // seq will add 1 after decrypt_incoming. do it mannually?
                // here we send to verifier the msg[ok], seq[~ok]
                // verifier can derive his key and iv by himself.
                // but how we can ensure it is not handshake?

                // HS ok
                if ret.typ == ContentType::Handshake && !is_decrypt_error {
                    println!("Hi Handshake");
                }

                // HS error or SATS error
                if ret.typ != ContentType::Handshake && is_decrypt_error {

                    // We intercept the ApplicationData here for TLS 1.2
                    // Note: Even under TLS 1.3, it is stated as a TLS 1.2 message.
                    if encrypted_msg.version == ProtocolVersion::TLSv1_2 
                    && ((args[10] == "TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256" || args[4] == "TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256" )
                    || (args[10] == "TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256" || args[4] == "TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256" ))
                    && encrypted_msg.typ != ContentType::Handshake 
                    && is_decrypt_error
                    {
                        println!("is_decrypt_error: {:?}", is_decrypt_error);
                        ...
                        let mut payload: Vec<u8> = encrypted_msg.payload.0;
                        println!("msg.payload.encode(): {:?}", payload);
                        ...
                        if args[2] == "www.yalealumni.yale.edu" {
                            let mut stream = TcpStream::connect(&target_ip_port.as_str()).unwrap();
                            let client_message = get_payload(&stream);
                            if client_message == String::from("Exit").as_bytes().to_vec() {
                                std::process::exit(0);
                            }
                        }
                    ...
                    // TLS 1.3
                        ...
                        println!("HS error or SATS error");
                        
                        // println!("TEST GLOBAL IP: {}", crate::VERIFIER_IP_WITH_PORT);
                        ...
                        stream.write(&signal_to_verifier).unwrap();
                        println!("signal_to_verifier: {:?}", signal_to_verifier);
                        ...
                        if client_message == String::from("After SHTS").as_bytes().to_vec() {
                            // HS error
                            // Abort
                            use crate::Error::CorruptMessage;
                            return Err(CorruptMessage);
                        }
                        else if client_message == String::from("After SATS").as_bytes().to_vec() {
                            // SATS error
                            // send payloads to verifier
                            println!("ret.typ: {:?}", ret.typ);
                            println!("send the non-handshake opaque msg");
                            println!("{:?}", &encrypted_msg.payload);

                            // send opaque msg
                            let encr_payload = encrypted_msg.payload.0;
                            // let encr_payload = serde_json::to_string(&encrypted_msg.payload).unwrap();
                            println!("encr_payload: {:?}", &encr_payload);
                            // let mut stream = TcpStream::connect(crate::TARGET_IP_WITH_PORT).unwrap();
                            let mut stream = TcpStream::connect(&target_ip_port.as_str()).unwrap();
                            stream.write(&encr_payload).unwrap();
        ...
        && is_decrypt_error
        {
            println!("TLS 1.2 Handshake is_decrypt_error");
        }
        ...
        println!("Before self.handshake_joiner.want_message(&msg)");
        // For handshake messages, we need to join them before parsing
        // and processing.
        if self.handshake_joiner.want_message(&msg)  {
            println!("handshake_joiner (&msg) -> True\n");

            // println!("\nhere we save the continue msg");
            // use std::io::Write;
            // let mut file = std::fs::File::create("./rustls/src/opaque_msg/enc_msg.txt").expect("create failed");
            // let encr_payload = String::from("continue");
            // file.write_all(&encr_payload.as_bytes()).expect("write failed");

            // println!("\nhere we send the continue msg to verifier");
            // let mut stream = TcpStream::connect(VERIFIER_IP_WITH_PORT).unwrap();
            // stream.write(&encr_payload.as_bytes()).unwrap();
            ...
            println!("handshake_joiner (&msg) -> false\n");
        //     println!("\nhere we send the continue msg to verifier");
        //     let encr_payload = String::from("continue");
        //     let mut stream = TcpStream::connect(VERIFIER_IP_WITH_PORT).unwrap();
        //     stream.write(&encr_payload.as_bytes()).unwrap();

        //     println!("\nhere we save the opaque msg");
        //     use std::io::Write;
        //     let mut file = std::fs::File::create("./rustls/src/opaque_msg/enc_msg.txt").expect("create failed");
        //     let encr_payload = serde_json::to_string(&encrypted_msg.payload).unwrap();
        //     file.write_all(&encr_payload.as_bytes()).expect("write failed");
        //     // println!("\nhere we need to send the verifier the opaque msg");
        //     // println!("{:?}", &encrypted_msg);
        //     // let encr_payload = serde_json::to_string(&encrypted_msg.payload).unwrap();
        //     // let mut stream = TcpStream::connect(crate::VERIFIER_IP_WITH_PORT).unwrap();
        //     // stream.write(&encr_payload.as_bytes()).unwrap();
        }

        // Now we can fully parse the message payload.
        // Decryption
        println!("msg before try_from: {:?}", msg.payload);
        println!("try_from");
        let msg = Message::try_from(msg)?;

        // For alerts, we have separate logic.
        // For normal TLS 1.2, if there is an error, 
        // it will return an error message and re-run the whole process (tlsclient.rs line ~152)
        // If it is ok, then proceed to next step.
        println!("payload");
        ...
        println!("process_main_protocol after payload checking");
        println!("self.common_state.received_plaintext: {:?}", self.common_state.received_plaintext);
...
impl CommonState {
    ...
    fn process_main_protocol<Data>(
        ...
        println!("Before state.handle");
        // hi expand will appear here inside handle()
        match state.handle(&mut cx, msg) {
            Ok(next) => {
                println!("state.handle OK");
                state = next;
                Ok(state)
            }
            Err(e @ Error::InappropriateMessage { .. })
            | Err(e @ Error::InappropriateHandshakeMessage { .. }) => {
                println!("state.handle Err");
    ...
    #[cfg(feature = "tls12")]
    pub(crate) fn start_encryption_tls12_deco(&mut self, secrets: &ConnectionSecrets) {
        println!("make_cipher_pair");
        let (dec, enc) = secrets.make_cipher_pair_deco();
        self.record_layer
            .prepare_message_encrypter(enc);
        self.record_layer
            .prepare_message_decrypter(dec);
    ...
    fn process_alert(&mut self, alert: &AlertMessagePayload) -> Result<(), Error> {
        println!("process_alert 1");
        // Reject unknown AlertLevels.
        if let AlertLevel::Unknown(_) = alert.level {
            self.send_fatal_alert(AlertDescription::IllegalParameter);
        }

        println!("process_alert 2");
        ...
        println!("process_alert 3");
        println!("self.is_tls13(): {:?}", self.is_tls13());
        ...
        println!("process_alert 4");
        println!("TLS alert received: {:#?}", alert);
