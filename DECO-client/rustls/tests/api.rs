...
mod test_quic {
    ...
    fn test_quic_handshake() {
        ...
        println!("****** Below is a full handshake ******");
        ...
        let client_hs = step(&mut client, &mut server)
            .unwrap()
            .unwrap();
        assert!(compatible_keys(&server_hs, &client_hs));  // sometimes succeed
...
/*
        // 0-RTT handshake
        println!("****** Below is an 0-RTT handshake ******");
        ...
        step(&mut client, &mut server)
            ...
            .unwrap();
        step(&mut server, &mut client)
            .unwrap()     // sometimes succeed
            .unwrap();
        assert!(client.is_early_data_accepted());

        // 0-RTT rejection
        println!("****** Below is an 0-RTT rejection ******");
        ...
        // failed handshake
        println!("****** Below is a failed handshake ******");
